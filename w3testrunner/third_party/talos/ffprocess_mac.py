...
def GetPidsByName(process_name):
  ...
  command = ['ps -Ac']
  # BT: handle() sometimes blocks for some reasons. Switched to using communicate() instead.
  (data, _) = subprocess.Popen(command, stdout=subprocess.PIPE, universal_newlines=True, shell=True).communicate()
